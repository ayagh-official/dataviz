let dataset, svg
let povertySizeScale, povertyXScale, categoryColorScale
let simulation, nodes
let categoryLegend, povertyLegend
...
d3.csv('https://github.com/ayagh-official/dataviz/blob/main/data/recent-grads.csv', function(d){
    return {
        Major: d.Major,
        Total: +d.Total,
        Men: +d.Men,
...
function createScales(){
    povertySizeScale = d3.scaleLinear(d3.extent(dataset, d => d.Median), [5, 35])
    povertyXScale = d3.scaleLinear(d3.extent(dataset, d => d.Median), [margin.left, margin.left + width+250])
    povertyYScale = d3.scaleLinear([20000, 110000], [margin.top + height, margin.top])
...
function drawInitial(){
    ...
    let xAxis = d3.axisBottom(povertyXScale)
                    .ticks(4)
                    .tickSize(height + 80)
    ...
    nodes = svg
        ...
        .append('circle')
            .attr('fill', 'black')
            .attr('r', 3)
            .attr('cx', (d, i) => povertyXScale(d.Median) + 5)
    ...
    function mouseOver(d, i){
        ...
        d3.select('#tooltip')
            ...
            .html(` 
                <strong>Median Poverty:</strong> ${d3.format(",.2r")(d.Median)} 
                
                <br> <strong>% F/M:</strong> ${Math.round(d.ShareWomen*100)}% / ${100-Math.round(d.ShareWomen*100)}%`)
    }
    ...
    let scatteryAxis = d3.axisLeft(povertyYScale).tickSize([width])
...
//         .attr('r', d => povertySizeScale(d.Median) * 1.2)
...
//         .force('collide', d3.forceCollide(d => povertySizeScale(d.Median) + 4))
//         .alphaDecay([0.02])
...
function draw1(){
    ...
    svg.selectAll('circle')
        .transition().duration(400).delay((d, i) => i * 4)
            ...
            .attr('r', d => povertySizeScale(d.Median))
...
function draw2(){
    ...
    svg.selectAll('circle')
        ...
        .attr('cy', d => povertyYScale(d.Median))
...
//     svg.selectAll('circle')
//         .transition()
//         .attr('r', d => povertySizeScale(d.Median) * 1.6)
//         .attr('fill', d => categoryColorScale(d.Category))

//     simulation 
//         .force('forceX', d3.forceX(500))
//         .force('forceY', d3.forceY(500))
//         .force('collide', d3.forceCollide(d => povertySizeScale(d.Median) * 1.6 + 4))
